...
class CanvasRenderer {
    ...
    render() {
        ...
        // Draw NAS animation ball if active
        if (window.nasAnimationBall && window.nasAnimationBall.visible) {
            this.drawAnimationBall(window.nasAnimationBall);
        }
    }
 ...
 */
    ...
    drawConnection(conn) {
        ...
        // Hide connection line between UDM and UDR (connection exists logically, just not drawn)
        if ((sourceNF.type === 'UDM' && targetNF.type === 'UDR') ||
            (sourceNF.type === 'UDR' && targetNF.type === 'UDM')) {
            return;
        }
        ...
        // Draw interface name label at midpoint (skip for UDM <-> UDR)
        const hideInterfaceLabel =
            (sourceNF.type === 'UDM' && targetNF.type === 'UDR') ||
            (sourceNF.type === 'UDR' && targetNF.type === 'UDM');

        if (conn.interfaceName && !hideInterfaceLabel) {
 ...
 */
    ...
    /**
     * Draw animation ball for NAS steps
     * @param {Object} ball - Ball object with x, y, visible properties
     */
    drawAnimationBall(ball) {
        ...
        // Draw highlight
        this.ctx.beginPath();
        this.ctx.arc(ball.x - 2, ball.y - 2, radius / 3, 0, Math.PI * 2);

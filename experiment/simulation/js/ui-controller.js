...
class UIController {
    ...
    setupOneClickDeployButton() {
        ...
        btn.addEventListener('click', () => {
            this.deployOneClickTopology();
        });
    }

    /**
     * Deploy topology from one-click.json file (root folder)
     */
    async deployOneClickTopology() {
        try {
            const message = [
                '🚀 One-Click Topology Deployment',
                '',
                'This will load the topology defined in one-click.json:',
                '',
                '• Deploy all NFs (core, UPF, MySQL, gNB, UE, ext-dn, etc.) one by one',
                '• Then create all buses and connections automatically',
                '',
                '⚠️ WARNING: This will replace your current topology!'
            ].join('\n');

            if (!confirm(message)) {
                console.log('❌ One-click deployment cancelled by user');
                return;
            }
            ...
            if (window.logEngine && typeof window.logEngine.clearAllLogs === 'function') {
            ...
            // Load topology from one-click.json (project root)
            console.log('📄 Loading one-click.json topology...');
            const response = await fetch('../one-click.json');
            if (!response.ok) {
                throw new Error(`Failed to load one-click.json: ${response.status} ${response.statusText}`);
            }

            const topo = await response.json();
            console.log('✅ one-click.json loaded, deploying sequentially...');

            await this.deployTopologySequentially(topo);
        } catch (error) {
            console.error('❌ One-click deployment failed:', error);
            alert(`❌ One-click deployment failed:\n\n${error.message}`);
    ...
    /**
     * Setup Replay Logs button (5g-logs.json)
     */
    setupReplayLogsButton() {
        const btn = document.getElementById('btn-replay-logs');
        if (!btn) return;
    ...
    /**
     * Replay logs from 5g-logs.json in sequence
     */
    async replay5GLogs() {
        try {
            if (!window.logEngine) {
                alert('❌ Log engine not available.');
                return;
            }

            // Optional: confirm because this clears current logs
            const ok = confirm(
                '📜 Replay 5G Logs\n\n' +
                'This will clear the current log panel and then\n' +
                'replay all logs from 5g-logs.json in sequence.\n\n' +
                'Continue?'
            );
            if (!ok) return;

            // Clear existing logs
            if (typeof window.logEngine.clearAllLogs === 'function') {
                window.logEngine.clearAllLogs();
            }

            // Load 5g-logs.json (project root)
            const res = await fetch('../5g-logs.json');
            if (!res.ok) {
                throw new Error(`Failed to load 5g-logs.json: ${res.status} ${res.statusText}`);
            }
            ...
            if (logs.length === 0) {
                alert('No logs found in 5g-logs.json.');
                return;
            }

            // Compress real time to keep replay reasonable
            const speedFactor = 0.1; // 10x faster than real timestamps
            let prevTs = logs[0].timestamp;

            for (const entry of logs) {
                const gap = Math.max(0, entry.timestamp - prevTs);
                const delayMs = gap * speedFactor;
            ...
            console.log('✅ 5g-logs.json replay finished');
        } catch (err) {
            console.error('❌ Failed to replay 5g-logs.json:', err);
            alert(`❌ Failed to replay 5g-logs.json:\n\n${err.message}`);
        }
    }

    /**
     * Deploy NFs, buses and connections from topology JSON SEQUENTIALLY
     * @param {Object} topo - Parsed one-click.json topology
     */
    async deployTopologySequentially(topo) {
        const nfConfigs = topo.nfs || [];
        const busConfigs = topo.buses || [];
        const busConnConfigs = topo.busConnections || [];
        const connConfigs = topo.connections || [];

        const idMap = {};     // original NF id -> new NF id
        const busIdMap = {};  // original bus id -> new bus id

        if (window.logEngine) {
            window.logEngine.addLog('system', 'INFO',
                '🚀 One-click sequential deployment started', {
                nfCount: nfConfigs.length,
                connectionCount: connConfigs.length,
                busCount: busConfigs.length,
                busConnectionCount: busConnConfigs.length,
                sourceFile: 'one-click.json',
                timestamp: new Date().toISOString()
            });
        }

        // 1) Create buses sequentially (BUS FIRST as requested)
        for (let i = 0; i < busConfigs.length; i++) {
            const bcfg = busConfigs[i];
            if (!window.busManager) continue;

            const bus = window.busManager.createBusLine(
                bcfg.orientation || 'horizontal',
                bcfg.position,
                bcfg.length || 600,
                bcfg.name
            );
            ...
            bus.thickness = bcfg.thickness || bus.thickness;
            bus.color = bcfg.color || bus.color;
            bus.type = bcfg.type || bus.type;

            if (window.dataStore && typeof window.dataStore.updateBus === 'function') {
                window.dataStore.updateBus(bus.id, bus);
            }

            busIdMap[bcfg.id] = bus.id;

            // Small delay so user sees bus appear before NFs
            await this.delay(300);
        }

        // 2) Deploy all NFs one by one (core, UPF, gNB, UE, etc.) and connect to buses immediately
        for (let i = 0; i < nfConfigs.length; i++) {
            const cfg = nfConfigs[i];

            if (!window.nfManager) continue;

            const nf = window.nfManager.createNetworkFunction(cfg.type, cfg.position);
            if (!nf) continue;

            // Override properties from JSON, but keep same ID mapping
            nf.name = cfg.name || nf.name;
            nf.color = cfg.color || nf.color;
            nf.icon = cfg.icon || nf.icon;
            nf.status = cfg.status || 'stable';
            nf.statusTimestamp = cfg.statusTimestamp || Date.now();
            nf.config = {
                ...(nf.config || {}),
                ...(cfg.config || {})
            };

            if (window.dataStore && typeof window.dataStore.updateNF === 'function') {
                window.dataStore.updateNF(nf.id, nf);
            }

            idMap[cfg.id] = nf.id;

            // Immediately connect NF to any buses that reference it in topology
            if (window.busManager) {
                for (const bcfg of busConfigs) {
                    const mappedBusId = busIdMap[bcfg.id];
                    if (!mappedBusId) continue;
                    const busHasThisNFInTopo = Array.isArray(bcfg.connections) && bcfg.connections.includes(cfg.id);
                    if (busHasThisNFInTopo) {
                        window.busManager.connectNFToBus(nf.id, mappedBusId);
                    }
                }
            }
            ...
            // Short delay so NFs (including gNB, UE) appear one by one
            await this.delay(400);
        }

        // 3) Connect NFs to buses (bus connections) sequentially (skip if already connected)
        for (let i = 0; i < busConnConfigs.length; i++) {
            const bConn = busConnConfigs[i];
            const newNfId = idMap[bConn.nfId];
            const newBusId = busIdMap[bConn.busId];

            if (newNfId && newBusId && window.busManager) {
                const bus = window.dataStore?.getBusById(newBusId);
                // Skip if already connected (from immediate connection above)
                if (bus && Array.isArray(bus.connections) && bus.connections.includes(newNfId)) {
                    continue;
                }
                window.busManager.connectNFToBus(newNfId, newBusId);
                await this.delay(250);
            }
        }

        // 4) Create direct NF-to-NF connections sequentially
        for (let i = 0; i < connConfigs.length; i++) {
            const c = connConfigs[i];
            const srcId = idMap[c.sourceId];
            const dstId = idMap[c.targetId];

            if (srcId && dstId && window.connectionManager) {
                window.connectionManager.createManualConnection(srcId, dstId);
                await this.delay(300);
            }
        }
        ...
        // Final summary
        const nfCount = nfConfigs.length;
        const connCount = connConfigs.length;
        const busCount = busConfigs.length;
        const busConnCount = busConnConfigs.length;

        if (window.logEngine) {
            window.logEngine.addLog('system', 'SUCCESS',
                '✅ One-click topology deployed from one-click.json', {
                nfCount,
                connectionCount: connCount,
                busCount,
                busConnectionCount: busConnCount,
                sourceFile: 'one-click.json',
                timestamp: new Date().toISOString()
            });
        }
        ...
        alert(
            `✅ One-Click Topology Deployed!\n\n` +
            `• Network Functions: ${nfCount}\n` +
            `• Direct Connections: ${connCount}\n` +
            `• Service Buses: ${busCount}\n` +
            `• Bus Connections: ${busConnCount}\n\n` +
            `Loaded from: one-click.json\n\n` +
            `All NFs (including gNB and UE) were created one by one automatically.`
        );
    }
    ...
    setupWindowsTerminal(nf, terminalModal) {
        ...
        content.addEventListener('click', (e) => {
            if (currentInput && !this.iperf3Servers.has(nf.id)) {
                currentInput.focus();
            }
        });
        ...
        const handleInputKeydown = async (e) => {
            ...
            if (e.key === 'Enter') {
                ...
                commandLine.textContent = `C:\\${nf.name}>${e.target.value}`;
                ...
                const allNFs = window.dataStore ? window.dataStore.getAllNFs() : [];
                const nfIPs = allNFs.map(n => n.config.ipAddress).filter(Boolean);
                ...
                const pingTargets = nfIPs.map(ip => `ping ${ip}`);
                const iperf3Targets = nfIPs.map(ip => `iperf3 -c ${ip}`);
                const commands = [...baseCommands, ...pingTargets, ...iperf3Targets];
                ...
                if (matches.length === 0) {
                    ...
                    if (lcp.length > val.length) {
                        ...
                        cmdLine.textContent = 'C:\\' + nf.name + '>' + val;
